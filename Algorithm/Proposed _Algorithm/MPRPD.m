function NextIndex = MPRPD(PopObj,IMatrix,N,Type)
...
if Len_Level1<=N
    ...
    if Type==1
        ...
        % calculate the angle distance matrix
        NPopObj = PopObj./repmat(sqrt(sum(PopObj.^2,2)),1,M);
        DMatrix = acos(NPopObj * NPopObj')+1e4*eye(Num);
    end
